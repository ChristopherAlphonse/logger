...
async function main() {
  ...
  console.log('\n📊 Example 2: Memory Warning');
  logger.warn('Memory usage exceeded 85% threshold: 3.4GB/4GB allocated, consider increasing heap size or optimizing memory usage');
  
  await sleep(2000);

  // Example 3: API rate limit
  console.log('\n📊 Example 3: API Rate Limit');
  logger.error('HTTP 429 Too Many Requests: Rate limit exceeded for API key abc123, retry after 60 seconds');
  
  await sleep(2000);

  // Example 4: File operation
  console.log('\n📊 Example 4: File Operation Info');
  logger.info('Successfully processed batch job: 1247 records inserted, 23 records skipped due to validation errors, execution time: 2.3s');
  
  await sleep(2000);

  // Example 5: Authentication failure
  console.log('\n📊 Example 5: Authentication Failure');
  logger.warn('JWT token validation failed: token expired at 2025-08-16T10:30:00Z, current time: 2025-08-16T10:45:00Z');
  
  await sleep(2000);

  // Example 6: Debug message (should not be translated by default)
  console.log('\n📊 Example 6: Debug Message (not translated)');
  logger.debug('Executing SQL query: SELECT * FROM users WHERE last_login > ?', { params: ['2025-08-01'] });
